...
bool Redis::publish(int channel, string message) {
  // redisCommand是阻塞的，内部先调用redisAppendCommand()，再调用redisGetReply()
...
// redisCommand是阻塞的，但不能每次subscribe都阻塞，所以使用redisAppendCommand
bool Redis::subscribe(int channel) {
  // SUBSCRIBE命令本身会造成线程阻塞等待通道里面发生消息，这里只做订阅通道，不接收通道消息
  // 通道消息的接收专门在observer_channel_message函数中的独立线程中进行
  // 只负责发送命令，不阻塞接收redis
  // server响应消息，否则和notifyMsg线程抢占响应资源
  // redisAppendCommand仅仅是把命令写到缓存
  if (REDIS_ERR ==
      redisAppendCommand(this->_subcribe_context, "SUBSCRIBE %d", channel)) {
    cerr << "subscribe command failed!" << endl;
    return false;
  }
  // redisBufferWrite把命令从缓存发送到redis-server上
...
void Redis::observer_channel_message() {
  ...
  while (REDIS_OK == redisGetReply(this->_subcribe_context, (void **)&reply)) {
    ...
    if (reply != nullptr && reply->element[2] != nullptr &&
      ...
      // 给业务层上报通道上发生的消息 element[1]是channel element[2]是msg
